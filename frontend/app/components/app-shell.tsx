...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto min-h-dvh w-full bg-[#F9FAFB] md:max-w-6xl md:px-6">
      <nav className="fixed bottom-0 left-0 right-0 z-40 mx-auto flex w-full max-w-[520px] items-center justify-between border-t border-[#E5E7EB] bg-white/95 px-1.5 pb-4 pt-3 backdrop-blur sm:px-2 sm:pb-5 md:sticky md:top-4 md:mx-0 md:mt-4 md:max-w-none md:rounded-2xl md:border md:px-5 md:py-3 md:shadow-sm md:backdrop-blur-0">
        {navItems.map((item) => {
          const isActive =
            pathname === item.href ||
          ...
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10px] sm:text-xs ${isActive ? "font-semibold text-[#0F8A3B]" : "text-[#6B7280]"}`}>
                {item.label}
              </span>
            </Link>
          );
